...
export const createBoardRepo = (client: Database | Transaction = db) => ({
  // ============================================================================
  // Boards
  // ============================================================================
  ...
  findMany: async (params: {
    ...
    filter?: { type: "trainer" | "rider"; id: string };
    canRiderAdd?: boolean;
  }) => {
    const createFilter = () => {
      if (!params.filter) return {};
      if (params.filter.type === "trainer") {
        return { assignments: { trainerId: params.filter.id } };
      } else if (params.filter.type === "rider") {
        return { assignments: { riderId: params.filter.id } };
      }
      return {};
    };
    const boards = await client.query.boards.findMany({
      where: {
        ...
        ...createFilter(),
        ...(params.canRiderAdd ? { canRiderAdd: true } : {}),
      },
      with: boardExpansion,
    });
    return boards;
...
export const boardRepo = createBoardRepo();
