...
export namespace upload {
    ...
    export interface ConfirmLogoUploadParams {
        organizationId: string
        key: string
    }

    export interface ConfirmLogoUploadResponse {
        logoUrl: string
    }
    ...
    export class ServiceClient {
        ...
        public async confirmAvatarUpload(params: ConfirmAvatarUploadParams): Promise<ConfirmAvatarUploadResponse> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("POST", `/upload/avatar/confirm`, JSON.stringify(params))
            return await resp.json() as ConfirmAvatarUploadResponse
        }

        public async confirmLogoUpload(params: ConfirmLogoUploadParams): Promise<ConfirmLogoUploadResponse> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("POST", `/upload/logo/confirm`, JSON.stringify(params))
            return await resp.json() as ConfirmLogoUploadResponse
        }
        ...
        public async deleteUserAvatar(params: DeleteUserAvatarParams): Promise<void> {
            // Convert our params into the objects we need for the request
            const query = makeRecord<string, string | string[]>({
